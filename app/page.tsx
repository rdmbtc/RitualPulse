...
export default function RitualPulsePage() {
  const { 
    ...
    newBlockArrived,
    hasAgentTx,
  } = useRitualData(4000) // Poll every 4 seconds
  ...
  // Calculate intensity for visuals based on network IQ
  ...
  return (
    ...
    >
      ...
      {/* Connection status */}
      <div className="fixed bottom-6 left-6 z-50 font-mono text-sm">
      ...
      {/* Title */}
      <motion.div 
        className="fixed top-6 right-6 z-50 font-mono"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 0.3 }}
      >
        <div className="text-xl font-semibold text-foreground">
          Ritual<span className="text-primary">Pulse</span>
        </div>
      </motion.div>
